...
- auth: add email verification process
    - add a route to confirm the email
    - check if confirmation is not expired during confirmation
    - add a route to resend the confirmation email
    - add a route to force confirmation (admin only)
    - add tests for confirmation
    - allow to use a transaction in a use-case
- auth: add endpoints to manage the list of sessions (show, invalidate, etc)

- passwordless:
    - user user login with email only
